...
class Node(PasarGuardNode):
    ...
    async def add_backend(
        ...
        """Start one more core on a node that is already connected.

        The node keys its running cores by type and instance, so a Start for a
        core it is not already running is additive. start() cannot be reused
        here: it calls connect(), which tears down and recreates the node's
        background tasks — doing that once per extra core would restart log
        streaming and user sync every time.
        """
        timeout = timeout or self._default_timeout
        health = await self.get_health()
        if health is Health.INVALID:
        ...
        async with self._node_lock:
            ...
            if not info.started:
                raise NodeAPIError(500, "Failed to start the extra core")
