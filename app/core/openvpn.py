...
class OpenVPNConfig(dict):
    ...
    def _clean_server_directives(cls, value) -> list[str] | None:
        ...
        if not isinstance(value, list):
            raise ValueError("extra_server_directives must be a list of strings")  # noqa: TRY004 - pydantic renders ValueError as a 422; TypeError would escape as a 500
        cleaned: list[str] = []
        for line in value:
    ...
    def _validate_listeners(self, default_port: int, default_proto: str, server_subnet: str) -> list[dict]:
        ...
        if not isinstance(raw, list):
            raise ValueError("listeners must be a list")  # noqa: TRY004 - pydantic renders ValueError as a 422; TypeError would escape as a 500
        if len(raw) > 8:
        ...
        for entry in raw:
            if not isinstance(entry, dict):
                raise ValueError("each listener must be an object with 'port' and 'proto'")  # noqa: TRY004 - pydantic renders ValueError as a 422; TypeError would escape as a 500
