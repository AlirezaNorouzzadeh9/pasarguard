...
@cached()
async def openvpn_settings() -> settings.OpenVPNSettings:
    async with GetDB() as db:
        db_settings = await get_settings(db)

    return settings.OpenVPNSettings.model_validate(db_settings.openvpn or {})
...
async def refresh_caches() -> None:
    ...
    await openvpn_settings.cache.clear()
