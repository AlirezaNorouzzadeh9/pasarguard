                ...
                <div className="w-full flex-1 space-y-4">
                  ...
                  <FormField
                    ...
                    render={({ field }) => {
                      ...
                      type ListenerOverride = { port: number; proto?: string | null }
                      type OverrideValue = number | ListenerOverride[]
                      const overrides: Record<string, OverrideValue> =
                        field.value && typeof field.value === 'object' ? (field.value as Record<string, OverrideValue>) : {}

                      const commit = (next: Record<string, OverrideValue>) =>
                        field.onChange(Object.keys(next).length ? next : null)

                      // Single-endpoint cores (wireguard, or an openvpn core with one
                      // listener) keep the plain port box.
                      ...
                      // An openvpn core can serve several endpoints (e.g. UDP and TCP).
                      // This node may bind each on its own port, so edit them as a list;
                      // an untouched list means "use the core's ports" and is not stored.
                      const coreListeners = (core: CoreSimple): ListenerOverride[] =>
                        (core.listeners ?? []).map(l => ({ port: l.port, proto: l.proto ?? undefined }))
                      const listenersOf = (core: CoreSimple): ListenerOverride[] => {
                        const value = overrides[String(core.id)]
                        return Array.isArray(value) ? value : coreListeners(core)
                      }
                      const setListener = (core: CoreSimple, index: number, raw: string) => {
                        const defaults = coreListeners(core)
                        const n = parseInt(raw, 10)
                        const edited = listenersOf(core).map((l, i) =>
                          i === index ? { ...l, port: raw && !Number.isNaN(n) ? n : defaults[i]?.port ?? 0 } : l,
                        )
                        const next = { ...overrides }
                        if (edited.every((l, i) => l.port === defaults[i]?.port)) delete next[String(core.id)]
                        else next[String(core.id)] = edited
                        commit(next)
                      }
                        ...
                        <FormItem>
                          ...
                          <div className="flex flex-col gap-2 rounded-md border p-3">
                            {portable.map((core: CoreSimple) => {
                              const defaults = coreListeners(core)
                              const overridden = Array.isArray(overrides[String(core.id)])
                              if (core.type === 'openvpn' && defaults.length > 1) {
                                return (
                                  <div key={core.id} className="flex flex-col gap-1.5">
                                    ...
                                    {listenersOf(core).map((l, i) => (
                                      <div
                                        key={`${core.id}-${i}`}
                                        className={cn('flex w-full items-center justify-between gap-2 ps-3', dir === 'rtl' && 'flex-row-reverse')}
                                      >
                                        <span className="text-xs uppercase text-muted-foreground">{defaults[i]?.proto ?? ''}</span>
                                        <div className="w-28 shrink-0">
                                          <Input
                                            type="number"
                                            className="h-8 w-full"
                                            placeholder={defaults[i]?.port != null ? String(defaults[i].port) : t('nodeModal.default', { defaultValue: 'default' })}
                                            value={overridden ? l.port : ''}
                                            onChange={e => setListener(core, i, e.target.value)}
                                          />
                                        </div>
                                      </div>
                                    ))}
                                ...
                                <div key={core.id} className={cn('flex w-full items-center justify-between gap-2', dir === 'rtl' && 'flex-row-reverse')}>
                                  ...
                                  <div className="w-28 shrink-0">
                                    <Input
                                      ...
                                      value={typeof overrides[String(core.id)] === 'number' ? (overrides[String(core.id)] as number) : ''}
