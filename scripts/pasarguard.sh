...
set_pasarguard_panel_image() {
    ...
    while IFS= read -r service_name; do
        ...
        if [[ "$image_name" =~ ^(pasarguard/panel|ghcr.io/alirezanorouzzadeh9/pasarguard)([:@].*)?$ ]]; then
...
uninstall_pasarguard_docker_images() {
    local images
    images=$(docker images --format '{{.Repository}} {{.ID}}' | awk '$1 ~ /^(pasarguard\/panel|ghcr\.io\/alirezanorouzzadeh9\/pasarguard)(:|$)/ {print $2}' | sort -u)

    if [ -z "$images" ]; then
        colorized_echo yellow "panel images not found"
        return 0
    fi

    colorized_echo yellow "Checking panel images for removal..."
