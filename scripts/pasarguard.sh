...
bootstrap_pasarguard_shared_libs() {
    local fetch_repo="AlirezaNorouzzadeh9/pasarguard"
...
install_pasarguard_script() {
    FETCH_REPO="AlirezaNorouzzadeh9/pasarguard"
    ...
    github_install_script_from_repo "$FETCH_REPO" "scripts/pasarguard.sh" "pasarguard"
...
set_pasarguard_panel_image() {
    ...
    while IFS= read -r service_name; do
        ...
        if [[ "$image_name" =~ ^ghcr.io/alirezanorouzzadeh9/pasarguard([:@].*)?$ ]]; then
...
install_pasarguard() {
    ...
    FILES_URL_PREFIX="https://raw.githubusercontent.com/AlirezaNorouzzadeh9/pasarguard"
    COMPOSE_FILES_URL_PREFIX="https://raw.githubusercontent.com/AlirezaNorouzzadeh9/pasarguard/main/scripts/docker-compose"
    ...
    local target_image="ghcr.io/alirezanorouzzadeh9/pasarguard:${pasarguard_version}"
    if [ "$pasarguard_version" == "latest" ]; then
        target_image="ghcr.io/alirezanorouzzadeh9/pasarguard:latest"
    fi
...
install_command() {
    ...
    check_version_exists() {
        local version=$1
        repo_url="https://api.github.com/repos/AlirezaNorouzzadeh9/pasarguard/releases"
...
uninstall_pasarguard_docker_images() {
    ...
    if [ -z "$images" ]; then
        colorized_echo yellow "ghcr.io/alirezanorouzzadeh9/pasarguard images not found"
        return 0
    fi

    colorized_echo yellow "Checking ghcr.io/alirezanorouzzadeh9/pasarguard images for removal..."
...
update_pasarguard_script() {
    ...
    if ! github_install_script_from_repo "$FETCH_REPO" "scripts/pasarguard.sh" "pasarguard"; then
...
install_node_command() {
    ...
    if [ "$(id -u)" = "0" ]; then
        ...
        bash -c "$(curl -sL https://github.com/AlirezaNorouzzadeh9/pasarguard/raw/main/scripts/pg-node.sh)" @ install
    else
        colorized_echo blue "Running node installation with sudo..."
        sudo bash -c "$(curl -sL https://github.com/AlirezaNorouzzadeh9/pasarguard/raw/main/scripts/pg-node.sh)" @ install
    fi
