...
NODE_SERVICE_REPO="AlirezaNorouzzadeh9/pasarguardNode"
...
uninstall_node_docker_images() {
    local images
    images=$(docker images --format '{{.Repository}} {{.ID}}' | awk '$1 ~ /^(pasarguard\/node|ghcr\.io\/alirezanorouzzadeh9\/pasarguardnode)(:|$)/ {print $2}' | sort -u)

    if [ -z "$images" ]; then
        colorized_echo yellow "node images not found"
        return 0
    fi

    colorized_echo yellow "Checking node images for removal..."
...
update_node_script() {
    ...
    if ! github_install_script_from_repo "$FETCH_REPO" "scripts/pg-node.sh" "$APP_NAME"; then
...
install_command() {
    ...
    check_version_exists() {
        local version=$1
        repo_url="https://api.github.com/repos/AlirezaNorouzzadeh9/pasarguardNode/releases"
        if [ "$version" == "latest" ]; then
            # The fork publishes the image as :latest on GHCR; no GitHub
            # releases are required for the default install.
            return 0
        fi
